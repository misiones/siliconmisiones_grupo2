	...
	<div class="container">
       ...
       <div class="panel panel_default">      
            ...
            <div class="pull-right">
                <a href="addalojamientos.php" class="btn btn-sm btn-dark">AGREGAR HOSPEDAJE</a>
                <!--<button type="button" class="btn btn-warning"><a href="add.php">AGREGAR SERVICIO</a></button>-->
            </div>
           <?php
               if(isset($_GET['action']) == 'delete'){
                  $id_delete = intval($_GET['idhospedaje']);
                  $query = mysqli_query($mysqli, "SELECT * FROM hospedaje WHERE idhospedaje='$id_delete'");
                  if(mysqli_num_rows($query) == 0){
                  echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
                   }else{
                   $delete = mysqli_query($mysqli, "DELETE FROM hospedaje WHERE idhospedaje='$id_delete'");
                   if($delete){
          ...
          <div class="table-responsive">
           <table class="table table-sm table-condensed table-bordered table-hover">
            ...
            <th scope="col">Camas doble</th>
            <th scope="col">Camas indiv.</th>
            ...
            <tbody>
                ...
                include('conectar3.php');
                    ...
                    $sentencia = $mysqli->prepare("SELECT COUNT(*) as cantidad FROM hospedaje");
                    $sentencia->execute();
                    ...
                    $sentencia = $mysqli->prepare("SELECT * FROM hospedaje LIMIT $offset, $cantidadMaximaElementosPagina");
                     //############## REALIZO LA CONSULTA PARA OBTENER SOLO LOS ELEMENTOS DE LA PAGINA ACTUAL ##############
                    ...
                    {         ?>
                              ...
                              echo "<td scope='row'>".$fila['idhospedaje']."</td>
                                   ...
                                   <td>".$fila['preciopordia']."</td>
                                    <td>".$fila['estado']."</td>
                                   <td>".$fila['capacidad']."</td>
                                   <td>".$fila['tamano']."</td>
                                   <td>".$fila['cama_doble']."</td>
                                   <td>".$fila['camaindi']."</td>";
                               ?>
                                    <td>
                                    <div class="btn-toolbar" role="toolbar">
                                       <div class="btn-group mr-5" role="group">
                                          ...
                                          <?php
                                          echo "<a class='\page-link\' href=edithospedajes.php?id=".$fila['idhospedaje'].">EDITAR</a>
                                          </button>";
                                          ?> 
                                        </div>
                                        <div class="btn-group mr-5" role="group">
                                          <button type='button' class="btn btn-outline-warning my-2 my-sm-0"> 
                                           <?php
                                           echo "<a href=hospedaje.php?action=delete&idhospedaje=".$fila['idhospedaje'].">ELIMINAR</a>";
